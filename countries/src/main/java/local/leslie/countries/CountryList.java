...
{
	//make an array inside the class
	public ArrayList<Country> countryList = new ArrayList<>();

	//use the capitalized CountryList - using the contructor to seed data
	public CountryList()
	{
	...
	//methods that return one country
	public Country findCountry(CheckCountry tester)
	{
		//GO THROUGH LIST AND RETURN THE FIRST THAT RETURNS TRUE OTHERWISE RETURN NULL
	...
	//methods that return a list of countries
	public ArrayList<Country> getCountryList(CheckCountry tester)
	{
		//make a temp array list to hold only the countries we want returned
		ArrayList<Country> tempCountryList = new ArrayList<>();

		//now loop through all the countries and return only the ones that return true
		for(Country c : countryList)
		{
